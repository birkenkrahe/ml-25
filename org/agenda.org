...
* DONE Week 12: Finish Naive Bayes Case

- [X] *New test*: Naive Bayes (by Friday, April 11) - 4 more
- [X] *DataCamp lesson*: Last of "Supervised Learning" (Apr 11) - 4 more
- [X] *Solution* of the text mining challenge - did you manage?
- [X] *Bayesian Stats Rapid Review* - preview for this week's test
- [ ] *Lecture/practice:* Regression methods
- [ ] *DataCamp Course*: Complete "Unsupervised Learning in R" by May 9

** Solution: Text mining challenge

** Bayesian Statistics Rapid Review - Deriving Bayes' Formula
...
* DONE Week 13: Regression methods
#+attr_html: :width 400px :float nil:
[[../img/challenger.jpg]]

- [X] *DataCamp Course*: Complete "Unsupervised Learning in R" by May 9
- [X] *Project review* with Minto's SCQA method.
- [ ] *Lecture/practice:* Regression methods
- [ ] *Logistic regression* - Challenger catastrophe use case
...
** Project review (3rd sprint - abstract)

- Demo film: "[[https://vimeo.com/305393045][Thank You Barbara Minto]]" (3 min) [[https://tinyurl.com/minto-scqa][tinyurl.com/minto-scqa]]

- Write this right now without using any sources other that what's
  in your head right now:

  1. *Situation:* Everything knows it, and it motivates you (and your
     listeners) to be interested in this research.

  2. *Complication*: The problem that specifically caused you to
     undertake this project.

  3. *Question*: Your central research question - the one that you think
     you will answer with your upcoming presentation.

  4. *Answer*: Your results so far in logical order.

- Put your answer in this Google Docs file (find your project name):
  [[https://tinyurl.com/dsc305-project-abstract][tinyurl.com/dsc305-project-abstract]]

*** *SCQA Example: Why I Started Using a To-Do List App*

- *Situation:* I used to keep all my tasks in my head or scribbled on
  sticky notes.  Most days, I had a general idea of what I needed to
  do.

- *Complication:* But I kept forgetting important deadlines and small
  tasks slipped through the cracks.  It was stressful and things
  started piling up.

- *Question:* How can I stay organized and make sure nothing gets
  forgotten?

- *Answer:*

  1. I realized I needed a system to keep track of my tasks reliably.

  2. I chose a simple to-do list app that works on my phone.

  3. I started writing down every task as soon as I think of it.

  4. I check the app each day to review and plan my work.

  5. As a result, I stay organized, feel less stressed, and don’t
     forget things anymore.


** SCQA for my faculty colloquium "AI - Muse or Tool"
...
* DONE Week 14: Review regression / Colloquium talk

- [X] *Review:* Regression methods
- [X] *Logistic regression* - Challenger catastrophe use case
- [ ] Regression case study (insurance data)

** Review (ungraded popquiz): Regression methods

1. What does "correlation does not imply causation" mean? Example?
   #+begin_quote
   Just because two variables are statistically related (they move
   together) does not mean that one causes the other.

   For example, ice cream sales and drowning incidents both increase
   in summer - but eating ice cream does not cause drowning (except in
   very rare cases - it can never be excluded).

   In this example, there is a third factor, the hot weather,
   influencing both. Temperature is a *confounding* variable, creating a
   misleading association between ice cream sales and death by
   drowning data.
   #+end_quote

2. If you assume causation but you're not sure, what could you do?
   #+begin_quote
   - Control for confounding variables: Apply multiple regression and
     see if the correlation still holds: hold the confounder
     (e.g. temperature) constant and check the correlation between ice
     cream sales and death by drowning (it should disappear).
   - Conduct controlled experiments to directly test
     causation. Confounders are removed by random treatments.
   - Apply causal inference frameworks (e.g. directed acyclic graphs,
     or do-calculus invented by Pearl) to model & test causality.
   - Replicate the finding across different datasets, populations,
     conditions. Consistency providers stronger evidence.
   - Apply statistical tests designed to uncover correlations: however,
     in order to dis/prove causation, statisticians use many, many
     different tests at once.
   #+end_quote

3. Name at least one weakness of linear regression analysis?
   #+begin_quote
   - Strong assumptions about the causal relationship of data.
   - Does not handle missing data well (scrubbing required).
   - Linearity including outliers can be ignored.
   - Only works with numeric data.
   - Requires knowledge of statistics (the more the better).
   #+end_quote

4. After computing the trendline for a single dependent variable, you
   add a couple more variables and calculate a multiple regression
   line. You notice that your trendline (intercept and slope) have not
   shifted at all. What can you conclude?
   #+begin_quote
   That the variables you added are statistically independent from the
   single variable used in the first place. The new variable(s) are
   orthogonal to the original variable - meaning that there is no
   pattern of correlated (synchronized growth) at all.
   #+end_quote
   ...
   #+begin_src R :session *R* :results output :exports both
     ...
     y <- 2 * x1 + 1 + rnorm(n)  ## y = 2*x1 + 1 + noise

     ## Generate x2 to be orthogonal (uncorrelated) with x1
     x2 <- rnorm(n)

     ## Pearson correlation coefficient for x1,x2
     cor(x1,x2)
   #+end_src
   ...
   #+begin_src R :file plot2.png :session *R* :results graphics output file :exports both
     plot(x1, x2, main = "Scatterplot of x1 vs x2", xlab = "x1", ylab = "x2")
     abline(lm(x2 ~ x1), col = "red")  # should be nearly flat
   #+end_src

   #+RESULTS:
   [[file:plot2.png]]

5. How would the trendlines for the single and the multiple regression
   in this last case look like?
   ...
   #+begin_src R :file plot.png :session *R* :results graphics output file :exports both
     ...
     plot(x1, y,
          ...
          main = "Effect of Adding Orthogonal Variable",
          xlab = "x1", ylab = "y")
     abline(model1, col = "blue", lwd = 2, lty = 1)
     abline(a = coef(model2)[1],
            b = coef(model2)["x1"],
            col = "red", lwd = 2, lty = 2)
     legend("topleft",
            legend = c("Simple Regression", "Multiple Regression"),
...
- Not every problem is suited to being modeled by a line.
  #+begin_quote
  However, exponential growth can be modeled after a logarithmic
  transformation - to model population growth, compound interest,
  radioactive decay etc.
  #+end_quote

- The statistical assumptions are often violated in the real world.
  #+begin_quote
1. Linearity: Predictors and response variable are related
   linearly. There needs to be a linear equation that governs them.
2. Errors: The residuals are statistically independent of one
   another. For example time-series data are often correlated over
   time - GDP shocks in an economic model can be
   autocorrelated. Clustered data (e.g. students in the same school)
   are often correlated.
  #+end_quote

- How can you check for these conditions?
  #+begin_quote
  1. Visualize effects of one predictor while controlling for others.
  2. Create partial regression plots (~car~ package in R).
  3. Plot errors over time or across space to look for patterns.
  4. Durbin-Watson test (autocorrelation in time-series).
  #+end_quote

- The Generalized Linear Models (GLM) loosense the assumptions:

  1. The target variable can be non-normally distributed,
     non-continuous (to model categorical data).

  2. The Variance of the target can be be related to its mean
     (allowing the model to fit where predictors relate to predictions
     in a non-linear way).

- Most popular: Logistic regression, adapted to binary classification
  tasks.

* DONE Week 15: Regression case study
#+attr_html: :width 400px :float nil:
[[../img/wrap.png]]

- [X] *Course evaluation:* Take 10 minutes to evaluate this course!
- [X] *DataCamp*: Unsupervised learning course due Friday May 9th.
- [X] *Test 7*: Due Tuesday, 6th May (linear regression).
- [X] *Test 8:* Due Friday, 9th May (unsupervised learning).
- [X] *Grading*: Will drop the worst/missing test/home assignment.
- [X] *Final exam:* Optional, can only improve your grade.
- [X] *Final presentation*: Book your slot & complete the checklist.
- [X] *Wait!* What about unsupervised learning?
- [X] *Code along*: Regression case study (insurance data).

** DONE Wait! What about unsupervised learning?

- *Regression analysis* is the gateway into the logic, mathematics, and
  philosophy of machine learning and data science.

- Easy to understand *loss functions* (e.g. mean squared error), model
  fitting, overfitting, and normalization.

- *Linear models* are well known from all areas of business and science:
  You've already come across them and their implications in school.

- Most *real-world ML applications* (spam detection, housing price
  prediction, medical diagnosis, etc.) are supervised problems.

- Overview comparison:
  
  | Regression Analysis                 | Unsupervised Learning           |
  |-------------------------------------+---------------------------------|
  | Clear intro to supervised learning  | Broad applicability to raw data |
  | Familiar, builds on prior knowledge | Reveals hidden structures       |
  | Focused on goal-driven tasks        | Encourages exploration          |
  | Teaches evaluation and metrics      | Core for feature learning       |
  | Strong industry relevance           | Critical for modern ML systems  |

- Lesson for Lyon: Split the class into supervised/unsupervised ML.
** TODO Multi-linear Regression case study - code along

*Data*:
- Get the code along file: [[https://tinyurl.com/reg-case-codealong][tinyurl.com/reg-case-codealong]]
  #+begin_src bash :results output :exports both
    wget -O reg-case.org tinyurl.com/reg-case-codealong
  #+end_src

*Plan:*
1. *Tuesday:* Workflow, getting and exploring the data, training model
2. *Thursday*: Evaluating and improving the model, adding interaction

** TODO Review: Insurance data regression case study

1. Why is it difficult for health insurance companies to estimate
   individual medical expenses accurately?
   #+begin_quote
   Because the most costly conditions are rare and appear randomly,
   making them hard to predict.
   #+end_quote

2. What is the dependent variable in the insurance dataset model, and
   what does it represent?
   #+begin_quote
   ~expenses~; it represents the total medical cost charged to the
   insurance plan for each individual.
   #+end_quote

3. List three criteria you should check before applying linear
   regression to any dataset.
   #+begin_quote
   1) Missing data  
...
4. How can you check linearity of the dataset before applying linear
   regression?
   #+begin_quote
   By visualizing relationships with scatterplots or a correlation
   matrix to see if variables have a roughly linear trend.
   #+end_quote
...
5. Which variables are candidates for a linearity check and why?
   #+begin_quote
   Especially =age=, =bmi= and =children= count, because these are
   continuous numerical features, and it is at least unclear a priori
   if they fall or grow linearly with the =expenses=.
   #+end_quote

6. Can you write the code to check these three vs. =expenses= in one
   plot window? How do you interpret the results?
   #+begin_src R :file plot.png :session *R* :results graphics output file :exports both
     par(mfrow=c(2,2))
     plot(insurance$expenses ~ insurance$age) # slightly nonlinear
     plot(insurance$expenses ~ insurance$bmi) # clustered
     barplot(table(insurance$children)) # categorical
   #+end_src
   ...
   #+begin_src R :file plot.png :session *R* :results graphics output file :exports both
     par(mfrow=c(2,2))
     plot(insurance$expenses ~ insurance$age) # slight nonlinearity visible
     plot(insurance$expenses ~ insurance$bmi) # on its own non-linear - transform?
     barplot(table(insurance$children)) # scatterplot useless (categorical)?
   #+end_src

7. What does the distribution of the ~expenses~ variable suggest about
   the shape of the data?
   #+begin_quote
   The data are right-skewed; most people have expenses below $15,000,
   but there are some high-cost outliers.
   #+end_quote
