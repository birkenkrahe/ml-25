...
* Week 11: Naive Bayes case study with text mining
#+attr_html: :width 300px :float nil:
[[../img/naive1.png]] 

** NEXT 2nd sprint review feedback
...
** TODO Text mining review

Challenge: Do what we've done so far in a one-liner.

1) Remove all objects from current session
2) Load ~tm~
3) Read CSV source from ~file~ (URL: bit.ly/sms_spam_csv)
4) VectorSource
5) VCorpus
6) 
...
#+begin_src R :session *R* :results output :exports both
  rm(list=ls())
  library(tm)
  read.csv(file="https://bit.ly/sms_spam_csv") |>
    VectorSource() |>
    VCorpus() |>
    tm_map(FUN=removeNumbers) -> x
